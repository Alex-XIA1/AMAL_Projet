...
class Model_traj(nn.Module):
    def __init__(self,d1,d2,d3,d4,d5,d6,nb_hop):
        ...
        #self.g1_0 = nn.Sequential(nn.Linear(d1,d2),tanh,dropout,nn.Linear(d2,d2),tanh,dropout, nn.Linear(d2,d2), tanh,dropout, nn.Linear(d2,d3),tanh)
        #self.g1_1 = nn.Sequential(nn.Linear(d1,d2),tanh,dropout,nn.Linear(d2,d2),tanh,dropout, nn.Linear(d2,d2), tanh,dropout, nn.Linear(d2,d3),tanh)
        #self.g1_2 = nn.Sequential(nn.Linear(d1,d2),tanh,dropout,nn.Linear(d2,d2),tanh,dropout, nn.Linear(d2,d2), tanh,dropout, nn.Linear(d2,d3),tanh)
        # Using ModuleList for g1 layers
        self.g1_layers = nn.ModuleList([
            nn.Sequential(
                nn.Linear(d1, d2), tanh,
                nn.Linear(d2, d2), tanh,
                nn.Linear(d2, d2), tanh,
                nn.Linear(d2, d3), tanh
            ) for _ in range(3)
        ])
        self.D = nn.Sequential(nn.Linear(nb_hop*d5,d5),tanh,nn.Linear(d5,d5),tanh, nn.Linear(d5,d5),tanh, nn.Linear(d5,d6),softmax)
