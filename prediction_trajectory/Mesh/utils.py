...
def plot_loss_acc(list_loss, list_acc, plot_type):
    ...
    plt.plot(list_loss, label=f' Loss')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    ...
    # Plotting Accuracy
    plt.figure(figsize=(8, 6))
    plt.plot(list_acc, label=f' Accuracy')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    plt.ylim(bottom=0)
...
def plot_combined_precision_recall_curve(true_labels, pred_probs, num_classes, output_file):
    """
    Saves a combined Precision-Recall curve for all classes to a file.
    ...
    for i in range(num_classes):
        ...
        ap_score = average_precision_score(true_labels[:, i], pred_probs[:, i])
